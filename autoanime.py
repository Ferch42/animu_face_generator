...
def draw_glasses(im, shapes, gender):
    ...
    im.paste(glasses_im, box=(int(shift_x),int(shift_y) ), mask=glasses_im)
...
def detect_glasses(fname, shapes):
    ...
    mask = mask[:,int(mask.shape[1]/2 -img.shape[1]/13):int(mask.shape[1]/2 +img.shape[1]/13)]
...
def process_eyes(shape, orig_im, new_im):
    ...
    new_im.paste(l_anime_eye_resized, box=((int(lx - new_l_w/2 - new_l_w/12), int(ly-new_l_h/9))), mask=l_anime_eye_resized)
    new_im.paste(r_anime_eye_resized, box=((int(rx - new_r_w/2 + new_r_w/12), int(ry-new_r_h/9))), mask=r_anime_eye_resized)
...
def draw_eyebrows(shape, im, bushy, color):
    ...
    if not bushy: #draw lines
        for i in range(len(l_eyebrow)-1):
...
def add_ears(shape, im, colors):
    ...
    new_im.paste(left_ear, box=(int((shape.part(0).x-new_lw+new_lw/8)), int(shape.part(17).y+new_lh/5)), mask=left_ear)
    new_im.paste(right_ear, box=(int ((shape.part(16).x-new_rw/3)), int(shape.part(26).y+new_lh/6)), mask=right_ear)
...
def render_big_eyebrows(left, right): 
    ...
    cropped_l = energy_l[int(mid_h-(h/4)):int(mid_h+(h/4)), :]
    cropped_r = energy_r[int(mid_h-(h/4)):int(mid_h+(h/4)), :]
...
def resize(left, right):
    ...
    if (crop_l > 0):
        left = left[int(crop_l):int(new_h_left-crop_l) , :]
    if (crop_r > 0):
        right = right[int(crop_r):int(new_h_right-crop_r), :]
    return left, right        
...
def match_anime(left_r, right_r):
    ...
    Image.fromarray(np.array(left_r_bw, dtype = np.uint8)).save("olho_esquerdo_mask.jpg")
    left_r_normed = (left_r_bw - left_r_bw.mean()) / left_r_bw.std()

    right_r_bw = rgb2gray(right_r)
    Image.fromarray(np.array(right_r_bw, dtype = np.uint8)).save("olho_direito_mask.jpg")
